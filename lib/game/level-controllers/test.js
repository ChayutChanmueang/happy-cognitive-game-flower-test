...
.defines(function(){
    EntityLevelTest = ig.Entity.extend({
        ...
        showChoices:function(){
            ...
            for(var i = 0; i < 2; i++){
                for(var j = 0; j < 2; j++){
                    ...
                    if(ig.ua.mobile){
                        var button = ig.game.spawnEntity(EntityChooseButton,((ig.system.width/2) - 205) + (j * (200 + 10)) ,(ig.system.height/2) + 230 + (i * 75),{anchor:2,value:randomValue});
                    }
                    else{
        ...
        draw: function(){
            ...
            ctx.fillStyle = "black";
            ctx.font = 40  * _GAMESETTINGS.screenScale + "px NotoSansThai";
            ctx.textAlign = "center";
            if(ig.ua.mobile){
                ctx.fillText("รอบที่", (ig.system.width/2) * _GAMESETTINGS.screenScale, 100 * _GAMESETTINGS.screenScale);

                ctx.font = 30  * _GAMESETTINGS.screenScale + "px NotoSansThai";
                ctx.textAlign = "center";
                ctx.fillText(this.currentRound.toString()+"/"+this.maxRound, (ig.system.width/2) * _GAMESETTINGS.screenScale, 145 * _GAMESETTINGS.screenScale);
            }
            else{
                ctx.fillText("รอบที่", (ig.system.width/2) * _GAMESETTINGS.screenScale, 75 * _GAMESETTINGS.screenScale);
