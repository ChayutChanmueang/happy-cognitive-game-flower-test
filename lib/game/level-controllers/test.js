...
.defines(function(){
    EntityLevelTest = ig.Entity.extend({
            ...
            if(this.randomRound < 4){
                ...
                if(this.cooldownTimer.delta() > 0){
                    ...
                    if(!this.alreadyDisplayChoice){
                        ...
                        this.title = ig.game.spawnEntity(EntityText,(ig.system.width/2) - 185,(ig.system.height/2) + 250,{text:"How much flower did appear?",size:"30"})
                        //spawn buttons here
                        this.currentChoices = [];
                        for(var i = 0; i < 2; i++){
                            for(var j = 0; j < 2; j++){
                                ...
                                var button = ig.game.spawnEntity(EntityChooseButton,10 + (j * (200 + 10)) ,(ig.system.height/2) + 300 + (i * 75),{value:randomValue});
        ...
        onCorrectAnswer:function(){
            console.log("Correct");
            this.answerIndication = ig.game.spawnEntity(EntityText,(ig.system.width/2) - 25,300,{text:"Nice !",size:"30"});
            this.correctRound++;
            if(this.currentRound < this.maxRound){
        ...
        onWrongAnswer:function(){
            console.log("Wrong");
            this.answerIndication = ig.game.spawnEntity(EntityText,(ig.system.width/2) - 50,300,{text:"Wrong !",size:"30"});
            this.wrongRound++;
            if(this.currentRound < this.maxRound){
                this.roundCooldown = new ig.Timer(1);
            }
